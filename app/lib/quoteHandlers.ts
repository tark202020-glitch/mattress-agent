...
export const handleDownloadQuoteWithCondition = async (
    designState: any, custom: any,
    ...
    try {
        ...
        const summaryItems = [
            { label: '상단폼', value: designState.topFoamEnabled === true ? (topFoam?.label || '선택중') : designState.topFoamEnabled === false ? '미적용' : null },
            { label: '가드폼', value: designState.guardFoamEnabled === true ? `${designState.guardFoamThickness}mm / ${designState.guardFoamHardness}` : designState.guardFoamEnabled === false ? (designState.isDual ? `중앙 구분만 (${designState.guardFoamThickness}mm)` : '미적용') : null },
            { label: '하단폼', value: designState.bottomFoamEnabled === true ? `${designState.bottomFoamThickness}mm / ${designState.bottomFoamHardness}` : designState.bottomFoamEnabled === false ? '미적용' : null },
        ...
        const specsList = summaryItems
            .filter(item => item.value !== null)
            .map(item => `- ${item.label}: ${item.value}${(item as any).sub ? ` (${(item as any).sub})` : ''}`);
        ...
        const addSizeData = (label: string, w: number, d: number) => {
            const tempState = { ...designState, customWidth: w, customDepth: d };
            const tempSummary = calculateSummary(tempState);
            const costPrice = tempSummary.totalUnitPrice;
            // 비율 + 마진 적용
            const subtotal = costPrice * (1 + (laborRate + materialRate + salesRate) / 100);
            const finalPrice = Math.round(subtotal * (1 + marginRate / 100));
            sizeData.push({ label, w, d, h: 250, price: finalPrice });
        };
        ...
        const res = await fetch('/api/quote/generate', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ coverName, title, specsList, sizeData })
        });
