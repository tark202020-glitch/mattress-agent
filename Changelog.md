## [Alpha V1.090] - 2026-09-07 08:58:32
...
- **Build Time**: 2026-09-07 08:58:32
...
### 🐛 Bug Fix
- **Summary**: 포장(7단계) 완료 후 배송(8단계) 미진입 버그 수정
- **Detail** : 
  - **`app/lib/store.ts` [MODIFY]**: 센서 단계 추가로 인해 총 스텝이 8단계로 확장되었으나, 다음 단계 이동(`nextStep`) 로직의 최대 허용치가 `Math.min(..., 7)`로 남아있어 배송 단계로 넘어가지 못했던 오류를 `Math.min(..., 8)`로 상향 수정하여 정상화했습니다.
- **Build Time**: 2026-03-06 12:40:00
...
### 🔄 Build Update
- **Summary**: 컨셉이미지 생성 모달 추가 및 AI 커버 생성 로직 스펙 동기화
- **Detail** : 
  - **`app/components/ConceptImageGeneratorModal.tsx` [NEW]**: 기존 브로셔 배경 생성과 유사하게 프롬프트를 확장했던 '컨셉이미지 생성' 전용 모달 추가
  - **UI/UX 개선**: 
    - AI 커버 생성 모달과 동일한 세로 정렬 표출(Flex-column 레이아웃 탑재).
    - 3개의 앵글(정면/퍼스펙티브/디테일) 생성 동기화 및 선택시 1:1, 2048x2048 해상도 일치.
  - **Auto-Save(자동저장) 동기화**: CoverImageGeneratorModal과 동일하게 3장 생성 직후 `resource/AI-concept` 폴더에 `base64`, `coverLabel`, `angleId` 파라미터 조합으로 자동 저장 구현
  - **디자이너 화면 수정**: AI 생성 이미지 존재 시에만 "컨셉이미지 생성" 활성화되도록 유효성 검사 적용. 배경 선택/카메라 앵글 프리셋 구역에 현재 적용되는 '작성된 프롬프트'를 직접 보컬-수정할 수 있게 읽기/쓰기 영역 4배 확장 개편.
- **Build Time**: 2026-02-28 18:25:00
